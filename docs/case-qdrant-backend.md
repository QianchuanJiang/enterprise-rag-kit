...
## 8. 要点总结
> “框架同时支持内存后端（零依赖、可演示）与 Qdrant 生产后端（持久化、payload 权限过滤、横向扩展），按客户算力与合规要求切换。本次把向量库从内存切到 Qdrant 后，检索/生成指标逐位一致，且验证了持久化（重启免入库即可检索）与 payload 级权限过滤——这正是政企‘数据不出域’私有化交付的标准形态。”
